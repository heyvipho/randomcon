...
func (tb *TB) Start() {
	...
	tb.i.Handle(telebot.OnPhoto, func(m *telebot.Message) { m.Photo.Caption = m.Caption; tb.forward(m.Sender, m.Photo) })
	tb.i.Handle(telebot.OnAudio, func(m *telebot.Message) { m.Audio.Caption = m.Caption; tb.forward(m.Sender, m.Audio) })
	tb.i.Handle(telebot.OnAnimation, func(m *telebot.Message) { m.Animation.Caption = m.Caption; tb.forward(m.Sender, m.Animation) })
	tb.i.Handle(telebot.OnDocument, func(m *telebot.Message) { m.Document.Caption = m.Caption; tb.forward(m.Sender, m.Document) })
	tb.i.Handle(telebot.OnSticker, func(m *telebot.Message) { tb.forward(m.Sender, m.Sticker) })
	tb.i.Handle(telebot.OnVideo, func(m *telebot.Message) { m.Video.Caption = m.Caption; tb.forward(m.Sender, m.Video) })
	tb.i.Handle(telebot.OnVoice, func(m *telebot.Message) { m.Voice.Caption = m.Caption; tb.forward(m.Sender, m.Voice) })
